...
void setup_mqtt (char *passAuth, char *orgID, char *deviceID, char *brokerPort){
	...
	if((settings->configuration.broker_port!=0) && (strcmp(settings->configuration.password,"")!=0) && (strcmp(settings->configuration.client_id,"")!=0) ){
		GOS_LOG("Porta: %d",settings->configuration.broker_port);
		GOS_LOG("PW: %s",settings->configuration.password);
		GOS_LOG("client: %s",settings->configuration.client_id);
