...
#define ERROR_400_BAD_FORMATTED "<h1>JSON not formatted correctly</h1>" \
												 "Must be of the form:<br />" \
												 "{\"ssid\":<ssid_str>,"\
												 "\"password\":<password_str>,"\
												 "\"orgID\":<orgID_str>,"\
												 "\"deviceID\":<deviceID_str>,"\
												 "\"passAuth\":<passAuth_str>,"\
												 "\"port\":<port_str>\"}"
...
//    uint16_t bytes_read;
	...
	{
		gos_hs_return_status_code(request, 400, ERROR_400_BAD_FORMATTED);
		goto exit;
	}
